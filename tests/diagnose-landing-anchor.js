...
for(const seed of [1,3,5,6,9]){
 ...
 for(let dy=0;dy<=6;dy++)for(let dx=-6;dx<=6;dx++){
   ...
   if(!rigid)continue;
   const phys=Math.hypot(ox*.5,oy*HEX_ROW_H);
   const noUp=oy<=1e-8;
 ...
 out.push({seed,piece:{x:g.piece.x,y:g.piece.y,rot:g.piece.rot},freeX:g.freeX,pieceVX:g.pieceVX,target:{x:target.x,y:target.y,cells:base.map(c=>c.slice(0,2))},shadow:shadow&&shadow.map(c=>c.slice(0,2)),candidates:candidates.slice(0,12)});
}
