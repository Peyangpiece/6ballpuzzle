...
for(const seed of [1,3,5,6,9]){
 ...
 for(let n=0;n<3+(seed%15);n++){const y=BOARD_MIN_ROW+Math.floor(rng()*(ROWS-BOARD_MIN_ROW)),xs=[];for(let x=0;x<W2;x++)if(valid(x,y)&&!g.board[y][x])xs.push(x);if(!xs.length)continue;const ball=mk(id++,Math.floor(rng()*COLORS.length));const x=xs[Math.floor(rng()*xs.length)];g.board[y][x]=ball;noteBoardCell(g.board,y,ball);}
