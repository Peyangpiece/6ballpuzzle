...
check(21,"hard drop lasts five source frames",close(HARD_DROP_VISUAL_TIME,5/30));
...
{const g=active(23);hardDrop(g);const a=g.hardDropAnim.fromY,z=g.hardDropAnim.target.y;g.hardDropAnim.t=g.hardDropAnim.dur*.5;const p=pieceSnapshotOf(g);check(23,"hard drop midpoint stays between endpoints",p.f+a-g.piece.y>=a-g.piece.y&&p.f<=z-g.piece.y);}
