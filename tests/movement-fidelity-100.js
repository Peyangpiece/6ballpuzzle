...
{const g=active(22),target=dropPiece(g.board,g.piece),cells=pieceCells(target);hardDrop(g);check(22,"instant drop commits the deepest contact in one transaction",g.state==="RESOLVING"&&!g.piece&&!g.hardDropAnim&&cells.every(([x,y])=>!!g.board[y][x]));}
{const g=active(23);setFreeX(g,g.piece.x+.43);g.pieceVX=g.freeX;hardDrop(g);const h=g.fx.hardDrops[0],s0=hardDropImpactState(h);h.life=h.max-2/30;const sp=hardDropImpactState(h);h.life=h.max-HARD_DROP_BEAM_DURATION;const se=hardDropImpactState(h);check(23,"instant drop preserves sub-cell X and uses measured impact phases",h.cells.every(c=>close(c[0]-Math.round(c[0]),.43,1e-6)||close(c[0]-Math.round(c[0]),-.57,1e-6))&&close(h.max,9/30)&&s0.beam>0&&sp.beam>s0.beam&&se.beam<1e-9);}
...
// 081-090: garbage bubble, cadence, continuous pre-contact fall and rendering.
...
{const g=createEngine(86);g.garbShapes=["PYRAMID"];prepareGarbageBatch(g);updateGarbagePacks(g,.01);const p=g.activeGarbagePacks[0];updateGarbagePacks(g,.4);let n=0;for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){const b=valid(x,y)?g.board[y][x]:null;if(b?.isGarbage)n++;}check(86,"garbage packet free-falls continuously before first contact",!p.landed&&p.y>GARBAGE_START_Y&&p.vy>0&&n===0);}
...
{const g=createEngine(90);g.activeGarbagePacks=[{type:"PYRAMID",seq:0,pat:[[0,0]],ax:4,y:-2,vy:0,bubbleT:.1,colors:[2],_started:true,landed:false}];const h=active(900);hardDrop(h);const fx=remoteFxSnapshotOf(h),r=createEngine(901);r.state="NET";applyRemoteVisualState(r,{piece:null,fx});check(90,"network snapshot includes moving garbage and instant impact",remoteFxSnapshotOf(g).g.length===1&&fx.d.length===1&&r.fx.hardDrops.length===1&&r.fx.hardDrops[0].seq===fx.d[0].seq);}
