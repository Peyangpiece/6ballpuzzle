...
(()=>{
 ...
 return{flags:{version:window.__sixBallRigidityReleaseBounceVersion,busy:window.__sixBallRigidityBlocksReplanWhileVisualBusy,resolverBusy:window.__sixBallResolverBlocksReplanWhileVisualBusy,settleBusy:window.__sixBallSettleBlocksReplanWhileVisualBusy,rebase:window.__sixBallFreshSegmentsStartAtRenderedCentre,pinned:window.__sixBallExternallyPinnedMemberBreaksTriplet,pair:window.__sixBallValidPairPivotMayRetainRigidity,upOuterBlocked:window.__sixBallReleaseBounceCannotSplitUpwardOuterContact,upPinnedBreaks:window.__sixBallExternallyPinnedUpwardTripletBreaks},bounce:noBounce(),stable:stableRelease(),differential:pinnedDifferential(),pair:pairPivot()};
})()
`,ctx);

console.log("RIGIDITY_RELEASE_BOUNCE",JSON.stringify(result));
expect(result.flags.version==="rigidity-release-bounce-authority-v1"&&result.flags.busy&&result.flags.resolverBusy&&result.flags.settleBusy&&result.flags.rebase&&result.flags.pinned&&result.flags.pair&&result.flags.upOuterBlocked&&result.flags.upPinnedBreaks===false,"release/bounce authority flags missing");
