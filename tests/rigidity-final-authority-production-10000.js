...
expect(ctx.__sixBallPositionFinalAlwaysReleasesRigidity===false,"position-final split trigger remains enabled");
...
expect(ctx.__sixBallUpConvexSplitOnlyCentralOrPositionFinal===false,"up-convex position-final trigger remains enabled");
...
expect(ctx.__sixBallStrictInnerSplitCarriesAuthorityToken===true,"strict inner split authority token missing");
...
expect(ctx.__sixBallUpConvexPositionFinalReleaseExemptsContactBand===false,"position-final band exemption remains enabled");
expect(ctx.__sixBallReleaseBounceCannotSplitUpwardOuterContact===true,"last release wrapper can still split an outer UP contact");
expect(ctx.__sixBallExternallyPinnedUpwardTripletBreaks===false,"externally-pinned wrapper still breaks upward triplets");
...
expect(positionFinalSplits===0,`position-final split path remains active in ${positionFinalSplits} cases`);
