...
const sourcePath=path.join(__dirname,"reference-fidelity.js");
const tmpPath=path.join(__dirname,".reference-fidelity-50-production-stack.js");
...
const runtimeDecl=/const runtimeNames\s*=\s*\[[\s\S]*?\];\nconst runtime\s*=\s*runtimeNames\.map\(read\)\.join\("\\n"\);/;
if(!runtimeDecl.test(source))throw new Error("50-point fidelity runtime declaration changed; update production runner explicitly");
