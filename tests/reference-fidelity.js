...
check(2,"ten/nine alternating columns",W2===19);
...
{const b=newBoard(),balls=[0,1,2].map(i=>({id:120+i,c:i,motionGroupId:12,motionGroupRole:i,motionGroupOrientation:"down",motionGroupSize:3,rigid:true})),m=[{ball:balls[0],x:0,y:2,role:0},{ball:balls[1],x:2,y:2,role:1},{ball:balls[2],x:1,y:3,role:2}];m.forEach(v=>b[v.y][v.x]=v.ball);const p=hexPhysPlanGroup(b,m,false);check(19,"wall adds zero rigidity",p.length===3&&balls.every(v=>v.rigid));}
{const b=newBoard(),balls=[0,1,2].map(i=>({id:130+i,c:i,motionGroupId:13,motionGroupRole:i,motionGroupOrientation:"down",motionGroupSize:3,rigid:true})),m=[{ball:balls[0],x:6,y:2,role:0,orientation:"down"},{ball:balls[1],x:8,y:2,role:1,orientation:"down"},{ball:balls[2],x:7,y:3,role:2,orientation:"down"}];m.forEach(v=>b[v.y][v.x]=v.ball);b[4][8]={id:139,c:4};const p=hexPhysPlanGroup(b,m,false);check(20,"slope keeps triplet rigidity",p.length===3&&p.every(v=>v.kind==="GROUP_SLOPE_TRANSLATE")&&balls.every(v=>v.rigid));}

{const g=createEngine(140),y=ROWS-1,b={id:140,c:1,motionGroupId:14,motionGroupRole:0,motionGroupOrientation:"down",motionGroupSize:1,rigid:true};g.board[y][1]=b;normalizeAllNonActivePileBalls(g);check(21,"settled pile rigidity is zero",!b.rigid&&!b.motionGroupId);}
{const b=newBoard(),balls=[0,1,2].map(i=>({id:150+i,c:i,motionGroupId:15,motionGroupRole:i,motionGroupOrientation:"up",motionGroupSize:3,rigid:true,impactOffsetX:-.5})),m=[{ball:balls[0],x:5,y:3,role:0,orientation:"up"},{ball:balls[1],x:6,y:4,role:1,orientation:"up"},{ball:balls[2],x:4,y:4,role:2,orientation:"up"}];m.forEach(v=>b[v.y][v.x]=v.ball);b[5][5]={id:159,c:4};const motions=m.map(v=>hexPhysIndependentMemberMotion(b,m,v));check(22,"centre-half split includes boundary",!!hexPhysUpConvexSeparator(b,m,motions));}
{const b=newBoard(),balls=[0,1,2].map(i=>({id:160+i,c:i,motionGroupId:16,motionGroupRole:i,motionGroupOrientation:"up",motionGroupSize:3,rigid:true,impactOffsetX:.51})),m=[{ball:balls[0],x:5,y:3,role:0,orientation:"up"},{ball:balls[1],x:6,y:4,role:1,orientation:"up"},{ball:balls[2],x:4,y:4,role:2,orientation:"up"}];m.forEach(v=>b[v.y][v.x]=v.ball);b[5][5]={id:169,c:4};const motions=m.map(v=>hexPhysIndependentMemberMotion(b,m,v));check(23,"outer quarter does not split",!hexPhysUpConvexSeparator(b,m,motions));}
{const b=newBoard(),balls=[0,1,2].map(i=>({id:170+i,c:i,motionGroupId:17,motionGroupRole:i,motionGroupOrientation:"down",motionGroupSize:3,rigid:true})),m=balls.map((ball,i)=>({ball,x:4+i*2,y:4,role:i,orientation:"down"}));m.forEach(v=>b[v.y][v.x]=v.ball);const oldMotion=hexPhysIndependentMemberMotion,oldSafe=hexPhysTranslationSafe;hexPhysIndependentMemberMotion=(board,group,v)=>v.role===0?null:{x:v.x,y:v.y,tx:v.x,ty:v.y+2,ball:v.ball,kind:"FREE_FALL",pivot:null,topPivot:null,followSupportIds:[]};hexPhysTranslationSafe=()=>true;const p=hexPhysPlanGroup(b,m,false);hexPhysIndependentMemberMotion=oldMotion;hexPhysTranslationSafe=oldSafe;check(24,"pinned member preserves rigid pair",p.length===2&&!balls[0].rigid&&balls[1].rigid&&balls[2].rigid&&balls[1].motionGroupSize===2);}
{const cells=GARBAGE_SHAPES.PYRAMID.map(v=>[v[0],v[1]]);check(25,"pyramid classification",classify(cells)==="PYRAMID");}
...
{const b=newBoard(),pat=GARBAGE_SHAPES.HEXAGON,ay=ROWS-3;pat.forEach(([x,y],i)=>b[ay+y][x]={id:180+i,c:2});check(28,"balanced hexagon hole retained",isBalancedHexagonCenterHole(b,2,ay+1));check(29,"balanced gap does not freeze",!boardHasIllegalFloat(b)&&!hasLegalGravityMove(b));}
...
{const g=createEngine(200);g.state="PLAYING";g.piece={x:8,y:-2,rot:0,colors:[0,1,2]};g.pieceVX=8;lock(g,3);check(45,"overflow deferred until equilibrium",g.alive&&g.state==="RESOLVING");}
{const a=createEngine(210);spawn(a);a.pieceVX=8.375;a.dropT=a.dropInterval*.4;a.fx.formations.push({w:"PYRAMID",cells:[[0,0]],tint:"#57FF7D",life:2,max:2.85});const p=pieceSnapshotOf(a),fx=remoteFxSnapshotOf(a),b=createEngine(211);b.state="NET";applyRemoteVisualState(b,{piece:p,fx});check(46,"remote active piece round-trip",b.piece&&close(b.pieceVX,8.375)&&b.piece.colors.join()==a.piece.colors.join());}
