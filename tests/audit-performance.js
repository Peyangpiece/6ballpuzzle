...
let frameToken=0,counts={resolve:0,relax:0,final:0,rests:0};
function wrap(name,key){if(typeof globalThis[name]!=='function')return;const old=globalThis[name];globalThis[name]=function(...args){counts[key]++;return old.apply(this,args);};}
...
for(const seed of [1,7,19,37]){
 ...
 for(let step=0;step<Math.floor(SECONDS/PHYSICS_FRAME)&&g.alive;step++){
  ...
  counts={resolve:0,relax:0,final:0,rests:0};const t=Date.now();stepEngine(g,PHYSICS_FRAME);const ms=Date.now()-t;frames++;sumMs+=ms;if(ms>maxMs)maxMs=ms;
