...
for(let i=0;i<1000;i++){
 ...
 if(mode===0){
  ...
  else if(kind===1){const target=dropPiece(g.board,g.piece),cells=pieceCells(target);hardDrop(g);const fx=g.fx.hardDrops[0];expect(g.state==="RESOLVING"&&!g.piece&&!g.hardDropAnim&&cells.every(([x,y])=>!!g.board[y][x])&&fx&&close(fx.max,9/30)&&sourceControls.includes("hardDrop(g)")&&!sourceControls.includes("commitCurrentColumn(g)"),"round2 reference instant drop "+i);}
