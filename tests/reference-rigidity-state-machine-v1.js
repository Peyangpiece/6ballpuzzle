...
(()=>{
  function installTriplet(game,specs,gid,orientation="up"){
    return specs.map((s,i)=>{
      const ball=mkBall(game,s.c??i);
      ball.motionGroupId=gid;
      ball.motionGroupRole=s.role??i;
      ball.motionGroupOrientation=orientation;
      ball.motionGroupSize=3;
      ball.rigid=true;
      ball.momentumX=s.bias||0;
      ball.rollDir=s.bias||0;
      ball.subCellBias=s.bias||0;
      ball.impactOffsetX=s.offset||0;
      game.board[s.y][s.x]=ball;noteBoardCell(game.board,s.y,ball);
      game.vis.set(ball.id,{x:s.vx??s.x,y:s.vy??s.y,vy:s.speed||0,motionSpeed:s.speed||0,justReleased:!!s.justReleased});
      return{ball,x:s.x,y:s.y,role:s.role??i,orientation};
    });
  }
  function maxDistanceError(members,plan){
    const byId=new Map(plan.map(p=>[p.ball.id,p]));let err=0;
    for(let a=0;a<members.length;a++)for(let b=a+1;b<members.length;b++){
      const ma=members[a],mb=members[b],d0=hexPhysDist(ma.x,ma.y,mb.x,mb.y);
      for(let i=0;i<=60;i++){
        const t=i/60,pa=byId.has(ma.ball.id)?proposalPointAt(byId.get(ma.ball.id),t):normPoint(ma.x,ma.y),pb=byId.has(mb.ball.id)?proposalPointAt(byId.get(mb.ball.id),t):normPoint(mb.x,mb.y);
        err=Math.max(err,Math.abs(Math.hypot(pa[0]-pb[0],pa[1]-pb[1])-d0));
      }
    }
    return err;
  }
  function freeFlight(){
    const g=createEngine(910001),members=installTriplet(g,[{x:6,y:2},{x:5,y:3},{x:7,y:3}],910001,"up");
    const plan=hexPhysPlanGroup(g.board,members,false)||[];
    return{count:plan.length,sizes:plan.map(p=>p.groupSize),sameBundle:new Set(plan.map(p=>p.bundleId)).size===1,rigid3:members.every(m=>m.ball.rigid&&m.ball.motionGroupSize===3),error:maxDistanceError(members,plan)};
  }
  function outerContact(seed,offset,raise=.0){
    const g=createEngine(seed),specs=[{x:6,y:3,c:2,role:0},{x:5,y:4,c:0,role:2},{x:7,y:4,c:4,role:1}],members=installTriplet(g,specs.map(s=>({...s,offset,bias:Math.sign(offset)||-1})),seed,"up");
    const support=mkBall(g,1);g.board[5][6]=support;noteBoardCell(g.board,5,support);g.vis.set(support.id,{x:6,y:5,vy:0,motionSpeed:0});
    ...
    for(const m of members)g.vis.set(m.ball.id,{x:m.x+offset,y:m.y+rowOffset,vy:5,motionSpeed:5,justReleased:true});
    g._visualMovingIds=new Set();g._liveBatchClock={elapsed:0,duration:0,states:new Map()};
    window.__sixBallLastNintendoRigidityDecision={};
    const plan=hexPhysPlanGroup(g.board,members,false)||[];
    const pairSteps=plan.filter(p=>Number(p.groupSize)===2),solo=plan.find(p=>Number(p.groupSize)===0),pairIds=pairSteps.map(p=>p.ball.id).sort((a,b)=>a-b);
    const pairMembers=members.filter(m=>pairIds.includes(m.ball.id));
  ...
  function pairPivot(){
    const g=createEngine(910101),fixed=mkBall(g,1),moving=mkBall(g,2),gid=910101;
    for(const [ball,role,x,y] of [[fixed,0,10,11],[moving,1,9,10]]){ball.motionGroupId=gid;ball.motionGroupRole=role;ball.motionGroupOrientation="pair";ball.motionGroupSize=2;ball.rigid=true;g.board[y][x]=ball;noteBoardCell(g.board,y,ball);g.vis.set(ball.id,{x,y,vy:0,motionSpeed:0});}
    const members=[{ball:fixed,x:10,y:11,role:0,orientation:"pair"},{ball:moving,x:9,y:10,role:1,orientation:"pair"}];
    const plan=hexPhysPlanGroup(g.board,members,false)||[];
    const p=plan[0]||null;
    return{count:plan.length,kind:p?.kind||null,target:p?[p.tx,p.ty]:null,pivot:p?.pivot||null,groupSize:p?.groupSize||0,fixedRigid:fixed.rigid,fixedSize:fixed.motionGroupSize,movingRigid:moving.rigid,movingSize:moving.motionGroupSize,error:plan.length?maxDistanceError(members,plan):99};
  }
  function settledTriplet(){
    const g=createEngine(910201),members=installTriplet(g,[{x:9,y:10},{x:8,y:11},{x:10,y:11}],910201,"up");
    ...
    const plan=hexPhysPlanGroup(g.board,members,false)||[];
    return{actual:actual.map(Boolean),independent:independent.map(Boolean),count:plan.length,released:members.every(m=>!m.ball.rigid&&m.ball.motionGroupSize===0&&m.ball.motionGroupId===0)};
  }
  return{
    ...
    free:freeFlight(),
    outerRight:outerContact(910301,-.82,0),
    outerLeft:outerContact(910302,.82,0),
    preRight:outerContact(910303,-.82,.08),
    pivot:pairPivot(),
    settled:settledTriplet()
  };
})()
`,ctx);
...
expect(result.flags.physical&&result.flags.kinematic&&result.flags.noMiddle50&&result.flags.noBilateral&&result.flags.distance&&result.flags.actualSettle&&result.flags.stableFirst&&result.flags.noStaticOverlap,"Nintendo rigidity contract flags incomplete");
...
expect(result.pivot.count===1,"2-ball rigid pivot did not produce one moving member");
