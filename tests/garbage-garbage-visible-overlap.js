...
function visibleGarbage(g,lead=0){
 ...
 for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){
  ...
  let px=v.x,py=v.y;
  if(lead>0&&!window.__hexGarbageVisibleOverlapGuard&&v.pileFlow&&Array.isArray(b.fallPath)&&b.fallPath.length){const p=pileFlowPositionAt(g,b,(g.pileFlowClock||0)+lead,0,null,memo);if(Number.isFinite(p?.[0])&&Number.isFinite(p?.[1])){px=p[0];py=Math.max(v.y,p[1]);}}
...
function buildPile(g,variant){for(let x=0;x<W2;x++)if(valid(x,ROWS-1))put(g,x,ROWS-1,(x+variant)%5);if(variant===0){put(g,5,ROWS-2,1);put(g,4,ROWS-3,2);}if(variant===1){put(g,3,ROWS-2,1);put(g,7,ROWS-2,2);put(g,5,ROWS-4,3);}if(variant===2){put(g,1,ROWS-2,1);put(g,5,ROWS-2,2);put(g,9,ROWS-2,3);put(g,4,ROWS-3,4);put(g,6,ROWS-3,0);}}
expect(window.__hexGarbageLocalConflictQueue===false,"predictive garbage queue is still enabled");
...
expect(typeof window.__hexGarbageLocalConflictIds==="function","garbage conflict API missing");
{
 ...
 expect(window.__hexGarbageLocalConflictIds(qg).size===0,"predictive same-destination hold was not disabled");
}
...
for(let variant=0;variant<3;variant++)for(const type of ["PYRAMID","HEXAGON","STRAIGHT"]){
 ...
 for(let frame=0;frame<180;frame++){
  ...
  const actual=minPair(visibleGarbage(g,0)),render=minPair(visibleGarbage(g,PHYSICS_FRAME));
