...
for(let seed=0;seed<300;seed++){
 ...
 const n=4+Math.floor(rng()*34);for(let k=0;k<n;k++){const y=BOARD_MIN_ROW+Math.floor(rng()*(ROWS-BOARD_MIN_ROW)),xs=[];for(let x=0;x<W2;x++)if(valid(x,y)&&!b[y][x])xs.push(x);if(!xs.length)continue;const x=xs[Math.floor(rng()*xs.length)];const ball=mk(id++,Math.floor(rng()*COLORS.length));b[y][x]=ball;noteBoardCell(b,y,ball);start++;}
 let guard=0;while(guard++<1000&&settlePass(b,false)){}
 let end=0;const ids=new Set();for(let y=BOARD_MIN_ROW;y<ROWS;y++)for(let x=0;x<W2;x++){const q=valid(x,y)?b[y][x]:null;if(!q)continue;end++;if(ids.has(q.id))fail('duplicate-id',{seed,id:q.id});ids.add(q.id);}
