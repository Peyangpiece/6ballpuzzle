...
// Static payload semantics used by App/local and online: shapes currently replace n.
for(const w of ['STRAIGHT','PYRAMID','HEXAGON']){
 ...
 for(let cancelled=1;cancelled<Math.min(total,10);cancelled++){
   const n=total-cancelled,delivered=effectiveDelivered(n,shapes);
   if(delivered!==n)bug('offset-payload-overdelivers',{w,total,cancelled,n,delivered,shapeBalls:shapeBalls(shapes)});
 }
}
...
for(const w of ['STRAIGHT','PYRAMID','HEXAGON'])for(const incoming of [1,5,11]){
 ...
 // Six valid cells are enough for CLEAR bookkeeping; technique label controls attack construction.
 ...
 if(effective!==expected)bug('real-clear-offset-delivery-mismatch',{w,incoming,expected,queuedN,queuedShapes,effective});
}
