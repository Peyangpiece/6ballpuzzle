...
put(g,6,11,0);put(g,8,11,1);put(g,10,11,2);put(g,7,10,3);put(g,9,10,4);
...
expect(window.__hexRuntimePerformanceVersion==="runtime-perf-v1","runtime performance adapter was not installed");
...
// A board version change must invalidate the collider cache while retaining
// exact canonical shadow geometry.
put(g,12,11,2);
...
expect(g._perfStaticLogicalColliders?.items!==cached1,"board change did not invalidate the collider cache");

// The moment any board ball is moving, every optimized wrapper must fall back
// to the canonical motion-aware routines.
...
console.log("runtime static-board performance guards PASS",JSON.stringify({staticContactSkips:g._perfStaticContactSkips||0,colliders:g._perfStaticLogicalColliders?.items?.length||0}));
`;
