...
/* ============================================================
 * FLOOR UP TRIANGLE LANDING v3.6
 *
 * AUTHORITATIVE FLOOR-ONLY RULE
 *
 *       TOP
 *    LEFT RIGHT
 *  ---------------- FLOOR
 *
 * When a normal UP triangle reaches the physical floor:
 *
 * - keep triangle geometry
 * - keep 3-ball rigidity through landing
 * - no 1+2 split
 * - no individual roll
 *
 * Once the position is finalized, ordinary settled-pile
 * finalization may remove motionGroup metadata.
 *
 * IMPORTANT:
 * This rule applies ONLY to the physical floor.
 * A ball-row surface is NOT handled here.
 * A real protrusion above the floor keeps the existing
 * authoritative split rule.
 * ============================================================ */
(function(){

    if(
        typeof window === "undefined" ||
        window.__sixBallFloorUpTriangleLandingV36
    ){
        return;
    }
    ...
    const basePlanGroupV36 =
        hexPhysPlanGroup;


    const baseSeparatorV36 =
        hexPhysUpConvexSeparator;


    function upLayoutV36(members){
    ...
    function onPhysicalFloorV36(
        ...
        /*
         * Both bottom members belong to the same floor row.
         */
        return(
            touchesFloorRow(
                g.left.y
    ...
    function maintainLandingRigidityV36(
        members
    ){

        const gid =
            members[0]?.ball?.motionGroupId ||
            members[1]?.ball?.motionGroupId ||
            members[2]?.ball?.motionGroupId ||
            0;
        ...
        for(const m of members){
            ...
            m.ball.__floorUpLandingRigidV36 =
                true;
        }


        window.__sixBallLastFloorUpLandingV36 = {
    ...
    /* ========================================================
     * FLOOR CANNOT BE A PROTRUSION SPLIT
     * ======================================================== */
     ...
     * FLOOR LANDING
     *
     * Stop as one rigid triangle.
     *
     * Returning [] means:
     * no further gravity event.
     *
     * It does NOT mean:
     * split the three balls.
     * ======================================================== */
    ...
    window.__sixBallFloorUpTriangleVersion =
        "floor-up-triangle-v3.6";

    window.__sixBallUpTriangleKeepsShapeOnFloor =
        true;

    window.__sixBallFloorCausesUpTriangleSplit =
        false;

    window.__sixBallBallRowRuleChangedByV36 =
        false;

    window.__sixBallProtrusionRuleChangedByV36 =
        false;
