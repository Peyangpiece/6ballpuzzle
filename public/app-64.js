...
function hex64BoardVisualObstacles(g){
    ...
    for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){
        ...
        out.push({
            ball,
            x:v&&Number.isFinite(v.x)?v.x:x,
            y:v&&Number.isFinite(v.y)?v.y:y
        });
    }
    return out;
}
function hex64PhysicalDropClearanceRows(g,piece,residual){
    if(!g||!piece)return 0;
    const H=HEX_ROW_H,obstacles=hex64BoardVisualObstacles(g);
    let rows=Infinity;
    for(const cell of pieceCells(piece)){
        const x=cell[0],y=cell[1];
        const mx=latticeRealX(x+residual),my=cellCenterYNorm(y);
        rows=Math.min(rows,Math.max(0,(FLOOR_CENTER_N-my)/H));
        for(const o of obstacles){
            const ox=latticeRealX(o.x),oy=cellCenterYNorm(o.y);
            const dx=Math.abs(mx-ox),dy=oy-my;
            ...
            const dr=(tangentY-my)/H;
            if(dr>=-HEX64_EPS)rows=Math.min(rows,Math.max(0,dr));
        }
    }
...
hexActivePhysicalDropPose=function(g,piece=g?.piece){
    ...
    let p={...piece};
    let remaining=hex64PhysicalDropClearanceRows(g,p,residual);
    const legacyFits=typeof __hexPieceFitsBeforeContinuousLegality==="function"
        ? __hexPieceFitsBeforeContinuousLegality
        : pieceFits;
    let guard=0;
    while(remaining>=2-1e-7&&guard++<ROWS+8){
        const next={...p,y:p.y+2};
        if(!legacyFits(g.board,next))break;
        p=next;remaining=Math.max(0,remaining-2);
    }
    const frac=Math.max(0,Math.min(2,remaining));
    return{piece:p,frac,dx:residual,cells:pieceCells(p)};
};
landingShadowVisualCells=function(g){
    const pose=hexActivePhysicalDropPose(g);
    if(!pose)return null;
    return pose.cells.map(cell=>[cell[0]+pose.dx,cell[1]+pose.frac,cell[2]]);
};
hardDrop=function(g){
...
function hex64GroupSnapshot(m){
    const b=m.ball;
    return{
        motionGroupId:b.motionGroupId,
        motionGroupRole:b.motionGroupRole,
        motionGroupOrientation:b.motionGroupOrientation,
        motionGroupSize:b.motionGroupSize,
        rigid:b.rigid,
        visualTripletId:b.visualTripletId,
        visualTripletOrientation:b.visualTripletOrientation,
        visualTripletRole:b.visualTripletRole
    };
}
...
hexPhysPlanGroup=function(board,members,preview=false){
    ...
    const snapshots=members.map(hex64GroupSnapshot);
    const full=__hex64PlanGroupBeforeMaxSubset(board,members,preview)||[];
    if(full.length)return full;

    // The wrapped planner may have cleared all group metadata while reporting a
    // failed common motion. Restore the pre-event triplet only for probing.
    for(let i=0;i<members.length;i++)hex64RestoreGroup(members[i],snapshots[i]);
    let physicalHit=0;
    for(const m of members)if(m.ball?._hexContinuousConstraintBreak){physicalHit=m.ball.id;break;}
    const candidates=[];
    for(let omit=0;omit<3;omit++){
        const pair=members.filter((_,i)=>i!==omit),plan=hex64ProbePair(board,pair);
    ...
    candidates.sort((a,b)=>
        (a.detached===physicalHit?-1:0)-(b.detached===physicalHit?-1:0)||a.detached-b.detached);
    const chosen=candidates[0];
    if(preview)return chosen.plan;
    ...
    chosen.pair.forEach((m,i)=>{
        m.ball.motionGroupId=gid;m.ball.motionGroupRole=i;m.ball.motionGroupOrientation=orientation;
        m.ball.motionGroupSize=2;m.ball.rigid=true;
    });
    const plan=__hex64PlanGroupBeforeMaxSubset(board,chosen.pair,false)||[];
    if(plan.length)return plan;
    for(const m of members){hexPhysClearGroupBall(m.ball);m.ball.rigid=false;}
    return[];
};
...
function hex64SetContinuousRestFromVisual(ball,v){
    ...
    ball._hexGarbageContinuousRest={
        px:v.x*.5,
        py:cellCenterYNorm(v.y),
        groupKey:old?.groupKey||ball._hexGarbageRelax?.groupKey||""
    };
}
function hex64RepairFirstSegmentGeometry(seg,from){
    if(!seg)return;
    seg.from=[from[0],from[1]];
    if(seg.pivot&&Math.abs(hexPhysDist(from[0],from[1],seg.pivot[0],seg.pivot[1])-1)>.03){
...
function hex64RescheduleRebasedSegment(g,seg,v){
    ...
    seg.pileFlowStart=Math.max(0,Number(g.pileFlowClock)||0);
...
function hex64SanitizeGarbagePath(g,ball){
    ...
    for(const seg of src){
        ...
        // Once a garbage ball has contacted the board, no obsolete logical
        // waypoint is allowed to pull its rendered centre upward again.
        if(target[1]<from[1]-HEX64_EPS){changed=true;continue;}
        if(needRebase){
            if(!seg.from||Math.abs(seg.from[0]-from[0])>HEX64_EPS||Math.abs(seg.from[1]-from[1])>HEX64_EPS){
                hex64RepairFirstSegmentGeometry(seg,from);hex64RescheduleRebasedSegment(g,seg,v);changed=true;
            }
            needRebase=false;
        }
    ...
    if(!kept.length&&!ball._hexGarbageRelax){
        hex64SetContinuousRestFromVisual(ball,v);
        delete ball._hexContinuousSettled;
    }
    return changed;
}
const __hex64PrepareContinuousPathBeforeMonotone=hexGarbagePrepareContinuousPath;
hexGarbagePrepareContinuousPath=function(g,ball,cell,from){
    const result=__hex64PrepareContinuousPathBeforeMonotone(g,ball,cell,from);
...
function hex64SanitizeAllGarbagePaths(g){
    ...
    for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){
        const ball=valid(x,y)?g.board[y][x]:null;
        if(ball&&hex64SanitizeGarbagePath(g,ball))n++;
    }
    return n;
}
function hex64SyncContinuousRests(g){
    ...
    for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){
        const ball=valid(x,y)?g.board[y][x]:null;
        if(!ball?._hexGarbageContinuousRest||ball._hexGarbageRelax||ball.fallPath?.length)continue;
...
stepEngine=function(g,dt){
    const prev=new Map();
    if(g?.board&&g?.vis)for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){
        const ball=valid(x,y)?g.board[y][x]:null,v=ball&&g.vis.get(ball.id);
        if(ball&&v&&Number.isFinite(v.y))prev.set(ball.id,v.y);
    }
    const result=__hex64StepBeforeFinalInvariants(g,dt);

    if(g?.state==="RESOLVING"&&g?.board&&g?.vis){
        // Reject any visible upward recoil of already-landed garbage before the
        // next frame can render it; then remove the obsolete upward path source.
        for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){
        ...
        hex64SanitizeAllGarbagePaths(g);
