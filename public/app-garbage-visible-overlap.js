 ...
 * ONLY that representation gap and the cosmetic render-lead gap. Board-to-board
 * garbage ordering/separation is owned by app-garbage-hard-separation so two
 * independent projection solvers can never fight each other.
 *
 * Same-batch garbage is collision-only here. It never becomes the surface that
 * triggers another airborne member's lattice entry.
 */
(function installGarbageVisibleOverlapGuard(){
    ...
    // A gridified garbage path must also sweep against unresolved packet members,
    // which are not present in g.board and are therefore invisible to the core
    // swept guard.
    ...
    // The unresolved rigid remainder is held at first tangent against gridified
    // garbage. This is collision-only and never invokes materialization.
    ...
    // Cosmetic renderLead bypasses collision resolution. Garbage therefore uses
    // the current resolved centre while ordinary pile keeps its future prediction.
