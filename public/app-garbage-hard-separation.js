 ...
 * This preserves the game's natural gravity/arc motion and cannot accumulate
 * sideways drift or hold unrelated balls in the air.
 */
(function installGarbageHardSeparation(){
    ...
    function boardItems(g){
        ...
        for(let y=boardScanMin(g.board);y<ROWS;y++)for(let x=0;x<W2;x++){
            ...
            if(ball?.isGarbage&&v&&Number.isFinite(v.x)&&Number.isFinite(v.y))out.push({ball,v,x,y});
        }
        return out;
    }
    function hardStationary(g,q){
    ...
    function firstUnsafeFraction(a0,a1,b0,b1){
        if(physicalDist(a0,b0)<MIN-1e-7)return null;
        let last=0;
        for(let i=1;i<=SAMPLES;i++){
            const t=i/SAMPLES,pa=pointAt(a0,a1,t),pb=pointAt(b0,b1,t);
            if(physicalDist(pa,pb)>=MIN-1e-9){last=t;continue;}
            let lo=last,hi=t;
            for(let k=0;k<BISECT;k++){
                const mid=(lo+hi)*.5;
                if(physicalDist(pointAt(a0,a1,mid),pointAt(b0,b1,mid))>=MIN)lo=mid;else hi=mid;
            }
            return Math.max(0,lo-1e-8);
        }
        return null;
    }
    function movementStart(q,before){
        const p=before.get(q.ball.id);if(p)return p;
        const seg=Array.isArray(q.ball.fallPath)&&q.ball.fallPath.length?q.ball.fallPath[0]:null;
        if(Array.isArray(seg?.from)&&Number.isFinite(seg.from[0])&&Number.isFinite(seg.from[1]))return[seg.from[0],seg.from[1]];
        return[q.v.x,q.v.y];
    }

    function clampGarbageFrame(g,before,dt){
        const list=boardItems(g);if(list.length<2)return;
        // Repeat because clamping one member can expose a second simultaneous
        // contact in a dense cluster. Rewinding along original paths cannot
        // create cumulative lateral drift.
        for(let pass=0;pass<20;pass++){
            let changed=false;
            for(let i=0;i<list.length;i++)for(let j=i+1;j<list.length;j++){
                const a=list[i],b=list[j],a1=[a.v.x,a.v.y],b1=[b.v.x,b.v.y];
                const a0=movementStart(a,before),b0=movementStart(b,before);
                const hardA=hardStationary(g,a),hardB=hardStationary(g,b);
                if(hardA&&hardB)continue;

                // A truly stationary pile ball stays fixed during the sweep.
                const sa=hardA?a1:a0,sb=hardB?b1:b0;
                const t=firstUnsafeFraction(sa,a1,sb,b1);
                if(t===null)continue;

                changed=true;
                const lost=Math.max(0,1-t)*Math.max(0,dt||0);
                if(!hardA){
                    const p=pointAt(sa,a1,t);a.v.x=p[0];a.v.y=Math.max(sa[1],p[1]);a.v.vy=0;a.v.motionSpeed=0;
                    a.v.garbageFrameContactClamped=true;a.v.garbageFrameContactClampCount=(a.v.garbageFrameContactClampCount||0)+1;
                    scheduleShift(a.ball,lost);
                }
                if(!hardB){
                    const p=pointAt(sb,b1,t);b.v.x=p[0];b.v.y=Math.max(sb[1],p[1]);b.v.vy=0;b.v.motionSpeed=0;
                    b.v.garbageFrameContactClamped=true;b.v.garbageFrameContactClampCount=(b.v.garbageFrameContactClampCount||0)+1;
                    scheduleShift(b.ball,lost);
                }
            }
            if(!changed)break;
        }
    ...
    updateVisuals=function(g,dt){
        const before=new Map();for(const q of boardItems(g))before.set(q.ball.id,[q.v.x,q.v.y]);
        const out=baseUpdateVisuals(g,dt);
        clampGarbageFrame(g,before,dt);
        return out;
    };

    window.__hexHardSeparateGarbage=function(g){
        const before=new Map();for(const q of boardItems(g))before.set(q.ball.id,[q.v.x,q.v.y]);
        clampGarbageFrame(g,before,0);
    };
