...
(function(){
    ...
    function inwardPocketCapture(board,members,g){
        ...
        for(const solo of g.lower){
            ...
            const ty=solo.y+1;
            const pocket=pocketAt(board,tx,ty,members);
            if(!pocket) continue;
    ...
    function isThreeBallRigidPlan(plan,members){
        ...
        for(const p of plan){
            if(!p?.ball || !ids.has(p.ball.id)) return false;
            if((p.groupSize||0)!==3) return false;
    ...
    function pairDirection(g,solo){
        return solo.ball.id===g.lower[1].ball.id ? -1 : 1;
    }
    ...
    function capturePlan(board,g,capture,preview){
        ...
        const pairLower=
            g.lower[0].ball.id===solo.ball.id ? g.lower[1] : g.lower[0];
    ...
    window.__sixBallUpConvexRigidUntilImpossibleVersion=
        "upconvex-rigidity-partial-release-v2.3";
    window.__sixBallUpConvexRigidImplementationVersion=
